...
const handler = NextAuth({
  ...
  callbacks: {
    async jwt({
      ...
      if (account?.access_token) {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_KEYCLOAK_BASE_URL}/realms/
        ...
        if (userInfo) console.log("useInfo,", userInfo);
        if (userInfo) {
          token.nama =
            userInfo.nama || userInfo.name || userInfo.preferred_username;
          token.nric = userInfo.nric
        }
      }
    ...
    async session({
      ...
      session.user.name = token.nama as string;
      session.user.nric = token.nric as string;
