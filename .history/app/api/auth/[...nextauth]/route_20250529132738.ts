...
const handler = NextAuth({
  providers: [
    KeycloakProvider({
      ...
      authorization: {
        url: `${process.env.NEXT_PUBLIC_KEYCLOAK_BASE_URL}/realms/${process.env.NEXT_PUBLIC_KEYCLOAK_REALM}/protocol/openid-connect/auth`,
        params: {
  ...
  callbacks: {
    async jwt({ token, account }: { token: JWT; account?: Account | null }): Promise<JWT> {
      if (account?.access_token) {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_KEYCLOAK_BASE_URL}/realms/${process.env.NEXT_PUBLIC_KEYCLOAK_REALM}/protocol/openid-connect/userinfo`, {
